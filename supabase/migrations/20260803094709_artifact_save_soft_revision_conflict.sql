-- Soft revision conflicts: return JSON instead of RAISE.
-- Stale client autosaves were flooding postgres ERROR logs (thousands/day)
-- with artifact_revision_conflict while optimistic concurrency still works.
...
begin
  ...
  if p_expected_version is not null and p_expected_version <> v_artifact.current_version then
    -- Soft conflict: do not RAISE (avoids postgres ERROR log stampede from
    -- stale TipTap autosaves). Client treats ok=false the same as before.
...
COMMENT ON FUNCTION public.ai_save_workspace_artifact_v2(
  ...
  'Optimistic artifact save with advisory lock; revision conflicts return ok=false JSON (no RAISE).';
