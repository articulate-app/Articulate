...
function markAttrs(marks: TipTapMark[] | undefined): Record<string, unknown> | undefined {
  ...
  for (const mark of marks) {
    ...
    if (!name) continue
    const nextAttrs = mark.attrs && Object.keys(mark.attrs).length > 0 ? mark.attrs : true
    attrs[name] = nextAttrs
  }
...
export function replaceYDocWithTipTapJson(
  ...
  document.transact(() => {
    ...
    if (content.length > 0) fragment.insert(0, content.map((child) => createYNode(child)))
  }, origin)
