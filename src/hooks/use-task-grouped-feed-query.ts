...
export interface UseTaskGroupedFeedQueryOptions {
  q: string;
  project?: string;
...
export interface UseTaskGroupedFeedQueryResult {
  groups: GroupedTaskGroup[];
  nextCursor: any | null;
  isFetching: boolean;
  hasMore: boolean;
  error: string | null;
...
export function useTaskGroupedFeedQuery({
  q,
  project,
  filters = {},
  groupBy,
  ...
  enabled = true,
  editFields,
}: UseTaskGroupedFeedQueryOptions): UseTaskGroupedFeedQueryResult {
  const [groups, setGroups] = useState<GroupedTaskGroup[]>([]);
  ...
  const buildRpcParams = useCallback(
    (cursor: any | null) => {
      ...
      return {
        ...
        p_groups_limit: groupsLimit,
        p_tasks_per_group: tasksPerGroup,
        p_cursor: cursor,
      };
    },
    [
  ...
  const performFetch = useCallback(
    async (cursor: any | null) => {
      ...
      console.log('[TaskGroupedFeed] Starting RPC fetch', {
      ...
      try {
        ...
        const { data, error: rpcError } = await supabase.rpc('task_grouped_feed_filtered', rpcParams);

        if (lastRequestIdRef.current !== requestId) {
          console.log('[TaskGroupedFeed] Ignoring stale RPC response');
          return;
        }
        if (queryKeyAtStart !== lastQueryKeyRef.current) {
          console.log('[TaskGroupedFeed] Query shape changed during fetch, discarding response');
          return;
        }

        if (rpcError) {
          console.error('[TaskGroupedFeed] RPC error:', rpcError);
          setError(rpcError.message || 'Failed to fetch grouped task feed');
          setHasMore(false);
          setNextCursor(null);
          return;
        }

        const payload = (data as { groups?: GroupedTaskGroup[]; next_cursor?: any }) || {};
        ...
        setGroups(prev => {
          ...
          const deduped = fetchedGroups.filter(g => !existingKeys.has(g.group_key));
          const combined = cursor ? [...prev, ...deduped] : deduped;

          console.log('[TaskGroupedFeed] Completed RPC fetch', {
        ...
        console.error('[TaskGroupedFeed] Unexpected error during RPC fetch', err);
        setError(err?.message || 'Failed to fetch grouped task feed');
        setHasMore(false);
        setNextCursor(null);
      } finally {
        if (lastRequestIdRef.current === requestId) {
    ...
    [buildRpcParams, groups.length, isEnabled],
  );
