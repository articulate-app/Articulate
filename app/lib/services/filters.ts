...
export async function getFilterOptions(): Promise<FilterOptions> {
  ...
  try {
    ...
    const dedupedStatuses = (statuses || []).filter(status => {
      if (!status.id || !status.name) return false
      const key = `${status.project_id}:${status.name}`
      if (uniqueStatusKeys.has(key)) return false
      uniqueStatusKeys.add(key)
      return true
    })
    const mappedStatuses = dedupedStatuses.map(status => ({ value: status.id, label: status.name, color: status.color, order_priority: status.order_priority, project_id: status.project_id }))
