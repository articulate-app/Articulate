...
const RichTextEditor = dynamic(() => import('../ui/rich-text-editor').then(mod => mod.RichTextEditor), { ssr: false })
...
export function AddTaskForm({
  ...
  useEffect(() => {
    ...
    // Only set default if the user hasn't selected a value
    if (watch('channels') && watch('channels').length > 0) return;
    let isMounted = true;
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (parentTaskId) {
        ...
        newTask = await addTask(subtaskPayload as any)
        addItemToStore('tasks', undefined, newTask)
        onSuccess?.(newTask)
      }
