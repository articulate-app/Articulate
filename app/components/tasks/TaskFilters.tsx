...
export function TaskFilters({ isOpen, onClose, onApplyFilters, activeFilters }: TaskFiltersProps) {
  const [filters, setFilters] = useState<TaskFilters>(activeFilters)
  const [statusNameToIds, setStatusNameToIds] = useState<Record<string, string[]>>({})
  const router = useRouter()
  const params = useSearchParams()

  // Fetch filter options only when panel is open
  const { data: options, isLoading: isOptionsLoading } = useFilterOptions({ enabled: isOpen })
  ...
  useEffect(() => {
    ...
    // Build mapping from name to all IDs for statuses
    const nameToIds: Record<string, string[]> = {}
    options.statuses.forEach((status: any) => {
      if (!nameToIds[status.label]) nameToIds[status.label] = []
      nameToIds[status.label].push(String(status.value))
    })
    setStatusNameToIds(nameToIds)
  }, [isOpen, options])
  ...
  const handleApplyFilters = () => {
    // Map selected status names to all their IDs
    const mappedFilters = {
      ...filters,
      status: filters.status.flatMap(name => statusNameToIds[name] || [])
    }
    onApplyFilters(mappedFilters, filters)
    syncFiltersToUrl(mappedFilters)
  }
      ...
      <ScrollArea className="flex-1 p-4">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <MultiSelect
              options={(options?.statuses || []).map(s => ({ id: s.label, label: s.label, color: s.color }))}
