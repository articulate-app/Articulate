...
interface AddExistingCreditNoteModalProps {
  invoiceId: number
...
export function AddExistingCreditNoteModal({ 
  invoiceId, 
  invoiceCurrency, 
  isOpen, 
  onClose, 
  onCreditNoteAdded 
}: AddExistingCreditNoteModalProps) {
  ...
  const { data: creditNotes, isLoading } = useQuery({
    queryKey: ['available-credit-notes', searchTerm, invoiceCurrency],
    queryFn: async () => {
      let query = supabase
        .from('v_credit_notes_summary')
        .select('*')
        .is('invoice_number', null) // Credit notes that haven't been allocated yet
                ...
                {creditNotes.map((creditNote: any) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      setAllocationAmount(creditNote.unallocated_amount)
                    }}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-500 mt-1">
                          Invoice: {creditNote.invoice_number} - {formatDate(creditNote.credit_date)}
                        </div>
                        {creditNote.reason && (
