            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={() => mutation.mutate("update")} disabled={mutation.isPending || rule.trim().length < 3}>
                  {mutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Check className="mr-2 h-4 w-4" />}Save
                </Button>
...
export function LearnedPreferencesSettingsPanel() {
  ...
  const createMutation = useMutation({
    mutationFn: () => createAiPreference({
      rule: newRule,
      category: newCategory,
      scope: view === "personal" ? "user" : newScope,
      projectId: view === "project" ? parsedProjectId : null,
    }),
    onSuccess: () => {
      setAdding(false)
      setNewRule("")
      setNewCategory("other")
      setNewScope(view === "project" ? "user_project" : "user")
      refresh()
    },
  })
    ...
    <div className="mx-auto w-full max-w-3xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-gray-500">Articulate learns durable preferences from your feedback and edits. Active preferences can influence future AI replies and artifacts; project knowledge and factual context remain separate.</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {view === "project" ? (
          <Select value={projectId} onValueChange={(value) => { setProjectId(value); setAdding(false) }}>
            <SelectTrigger className="w-[260px]"><SelectValue placeholder="Select a project" /></SelectTrigger>
            <SelectContent>{(projectsQuery.data ?? []).map((project) => <SelectItem key={project.id} value={String(project.id)}>{project.name}</SelectItem>)}</SelectContent>
          </Select>
        ) : null}
      ...
      {adding ? (
        <div className="space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Select value={newCategory} onValueChange={(value) => setNewCategory(value as AiPreferenceCategory)}>
            ...
            {view === "project" ? (
              <Select value={newScope} onValueChange={(value) => setNewScope(value as AiPreferenceScope)}>
                <SelectTrigger className="w-[210px]"><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="user_project">Personal · this project</SelectItem><SelectItem value="project">Shared project</SelectItem></SelectContent>
              </Select>
            ) : null}
            <Button size="sm" onClick={() => createMutation.mutate()} disabled={createMutation.isPending || newRule.trim().length < 3 || (view === "project" && !parsedProjectId)}>
              {createMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Check className="mr-2 h-4 w-4" />}Add
            </Button>
