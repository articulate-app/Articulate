...
/**
 * Full Task List as a workspace tab (middle or right).
 * Reuses TaskList / Calendar / Kanban + existing filter URL state — not a simplified list.
 */

import { useCallback, useEffect, useMemo, useRef, useState } from "react"
import { usePathname, useRouter, useSearchParams } from "next/navigation"
import { createClientComponentClient } from "@supabase/auth-helpers-nextjs"
import { Calendar, ChevronDown, Filter, LayoutGrid, List, Plus, Search } from "lucide-react"
...
function WorkspaceTaskListViewInner({ paneId }: { paneId: WorkspacePaneId }) {
  ...
  useEffect(() => {
    const v = (params.get("tasksView") || "list") as TaskListViewMode
  ...
  const commitFilters = useCallback(
    (
      newFilters: import("../tasks/TaskFilters").TaskFilters,
  ...
  const viewPill = (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          className="ml-1 inline-flex h-7 shrink-0 items-center gap-1.5 rounded px-2 text-[13px] font-medium text-gray-700 hover:bg-gray-100"
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col overflow-hidden bg-white">
      <div className="flex h-10 min-h-10 max-h-10 w-full flex-shrink-0 flex-nowrap items-center overflow-hidden border-b border-gray-200/80 bg-white">
        <div
          className={cn("flex min-w-0 flex-1 items-center", TOOLBAR_CONTENT_PX)}
        ...
        <div className={cn("flex flex-shrink-0 items-center pr-1.5", TOOLBAR_CONTENT_PX)}>
