...
interface GroupedTaskListProps {
  groups: { groupKey: string; groupLabel: string; trailingQuery: any }[]
  onTaskSelect?: (task: TaskListRow) => void
  pageSize?: number
}

export function GroupedTaskList({ groups, onTaskSelect, pageSize = 25 }: GroupedTaskListProps) {
  return (
    <div className="overflow-x-auto">
      {groups.map(group => (
        <div key={group.groupKey} className="mb-8">
          <div className="font-semibold text-lg px-2 py-2 bg-muted/50 border-b">{group.groupLabel}</div>
          <table className="border-collapse text-sm md:text-base w-full" style={{ tableLayout: 'fixed' }}>
            ...
            <InfiniteList<'tasks', TaskListRow>
              ...
              pageSize={pageSize}
              trailingQuery={group.trailingQuery}
              renderNoResults={() => (
                <tr>
