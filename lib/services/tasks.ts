...
export async function getTasks({
  ...
  try {
    ...
    if (filters.status?.length) {
      query = query.filter('project_statuses.name', 'in', `(${filters.status.join(',')})`)
    }
    ...
    const tasks: Task[] = (data || []).map((task: any) => ({
      ...
      project_statuses: Array.isArray(task.project_statuses) ? task.project_statuses.map((ps: any) => ({
        name: ps.name,
        id: ps.id ? Number(ps.id) : undefined
      })) : [],
