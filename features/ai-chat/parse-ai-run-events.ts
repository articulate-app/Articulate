import type { AiChatV2RunEvent, AiTargetKind } from "../../app/lib/ai/ai-chat-v2-types"
import { parseAiChatUsageSnapshot } from "../../app/lib/ai/ai-chat-usage-parse"
import type { AiClarificationOption, AiClarificationRequest } from "./ai-clarification"
...
export function ambiguousTargetToClarification(args: {
  event: Extract<AiChatV2RunEvent, { type: "ambiguous_target_confirmation_required" }>
  assistantMessageId?: string | null
}): AiClarificationRequest {
  const options: AiClarificationOption[] = args.event.candidates.map((candidate) => ({
    id: candidate.id,
    label: candidate.label,
  }))
  const question = args.event.question.trim()
  return {
    id: `${args.event.run_id}:${question}`,
    question,
    options,
